...
import os
import importlib
from logging import getLogger as _get_logger, Logger
import typing as t
...
_plugins: list[PluginClass] = []  # 已启用的插件类 (PluginClass) 列表 [plugin: PluginClass]
...
def get_logger(name: str) -> Logger:
    ...
    根据插件名返回对应 Logger
    '''
    return _get_logger(f'plugin.{name}')
...
def route(rule: str, *args, **kwargs):
    ...
    def decorator(func):
        ...
        _routes[f'/{plugin_name}{_rule}'] = (plugin_name, args, kwargs, func)
