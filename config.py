...
def get(_type, *key: str) -> ...:
    ...
    try:
        ...
        for k in key:
            value = value[k]
        assert isinstance(value, _type), ValueError
        return value
    except KeyError:
        pass
    except ValueError:
        raise SleepyException(f'Invaild config {".".join(key)} from config.yaml, it should be a(n) {_type.__name__} object.')
...
class _status:
    ...
    show_loading: bool = get(bool, _, 'show_loading')
